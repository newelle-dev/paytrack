...
import { ArrowLeft, Edit, Trash2, Phone, Mail, Clock, Calendar } from "lucide-react"
...
import { deleteBorrower, updateBorrower } from "@/app/(dashboard)/borrowers/actions"
...
export function BorrowerDetailClient({ initialData }: BorrowerDetailClientProps) {
  ...
  const allPayments = borrower.loans.flatMap(loan => 
    ...
    <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="col-span-1 p-6 flex flex-col gap-4 border border-ivory-cream bg-white shadow-sm rounded-lg">
          <h2 className="text-lg font-medium text-text-primary">Contact Details</h2>
          <div className="space-y-3">
            <div className="flex items-start gap-3 text-sm">
              <Mail className="h-4 w-4 mt-0.5 text-text-secondary" />
              <div>
                <p className="font-medium text-text-primary">Email Address</p>
                <p className="text-text-secondary">{borrower.email || "Not provided"}</p>
              </div>
            </div>
            <div className="flex items-start gap-3 text-sm">
              ...
              <div>
                ...
                <p className="text-text-secondary">{borrower.phone_number || "Not provided"}</p>
              </div>
            </div>
            <div className="flex items-start gap-3 text-sm border-t border-ivory-cream pt-3 mt-3">
              ...
              <div>
                ...
                <p className="text-text-secondary">{format(new Date(borrower.created_at), "MMMM d, yyyy")}</p>
              </div>
            </div>
          </div>
        </Card>
              ...
              <Table>
                ...
                <TableBody>
                  {allPayments.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center h-24 text-text-secondary">No payments found.</TableCell>
                    </TableRow>
                  ) : (
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Edit Borrower">
        <form onSubmit={handleEditSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input id="first_name" name="first_name" required defaultValue={borrower.first_name} disabled={pending} />
            </div>
            <div className="space-y-1.5">
              <label htmlFor="last_name" className="text-xs font-medium text-text-secondary">Last Name *</label>
              <Input id="last_name" name="last_name" required defaultValue={borrower.last_name} disabled={pending} />
            </div>
          </div>
          
          <div className="space-y-1.5">
            <label htmlFor="email" className="text-xs font-medium text-text-secondary">Email Address</label>
            <Input id="email" name="email" type="email" defaultValue={borrower.email || ""} disabled={pending} />
          </div>
          
          <div className="space-y-1.5">
            <label htmlFor="phone_number" className="text-xs font-medium text-text-secondary">Phone Number</label>
            <Input id="phone_number" name="phone_number" type="tel" defaultValue={borrower.phone_number || ""} disabled={pending} />
          </div>
          
          <div className="pt-4 flex justify-end gap-3">
            <Button type="button" variant="ghost" onClick={() => setIsEditModalOpen(false)} disabled={pending}>
              Cancel
            </Button>
