...
import { calculateSmallLoan, calculateBigLoan, LoanCalculationResult } from "@/lib/utils/loanMath"
...
export function LoanWizard({ initialBorrowers }: LoanWizardProps) {
  ...
  const [termType, setTermType] = useState<"Weekly" | "1-Month" | "Semi-Monthly" | "">("")
  ...
  const handleNextToStep3 = () => {
    ...
    if (isBigLoan) {
      result = calculateBigLoan(parsedPrincipal, releaseDate, Number(numMonths))
    } else {
      result = calculateSmallLoan(parsedPrincipal, releaseDate, termType as any)
    }
  ...
  const handleCustomAllocationChange = (type: "RC_$" | "EDITH_$" | "RC_%" | "EDITH_%", val: string) => {
    ...
    if (type === "RC_%") {
      ...
      newEdith = expected - newRc
      setEdithDollar(newEdith.toFixed(2))
      setRcPercent(expected > 0 ? ((newRc / expected) * 100).toFixed(2) : "0.00")
      setEdithPercent(expected > 0 ? ((newEdith / expected) * 100).toFixed(2) : "0.00")
    }
    else if (type === "EDITH_$") {
      newEdith = valNum
      newRc = expected - newEdith
    ...
    if (total !== expectedRounded) {
      setAllocationError(`Allocations must sum to ₱${expectedRounded.toFixed(2)}. Current sum: ₱${total.toFixed(2)}`)
    } else {
      setAllocationError("")
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4 text-sm font-medium">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className="flex items-center gap-2 flex-1 relative">
            <div className={`flex items-center gap-2 ${step >= s ? "text-primary" : "text-text-secondary"} bg-background z-10 pr-2`}>
              <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs ${step >= s ? "bg-primary text-primary-foreground" : "bg-bg-tertiary"}`}>
                {s}
              </div>
              <span className="hidden sm:inline">
                {s === 1 ? "Borrower" : s === 2 ? "Details" : s === 3 ? "Review" : "Confirm"}
              </span>
            </div>
            {s < 4 && <div className="absolute top-1/2 left-0 w-full h-px bg-border -z-0" />}
          </div>
        ))}
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {step === 1 && "Choose an existing borrower to assign this loan to."}
            {step === 2 && "Enter the principal amount and repayment terms."}
            {step === 3 && "Review the auto-generated schedule and profit allocation."}
            {step === 4 && "Finalize the details before saving to the database."}
          </CardDescription>
        </CardHeader>
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              {parsedPrincipal > 0 && parsedPrincipal <= 5000 && (
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="Semi-Monthly">Semi-Monthly (2 payments)</option>
                  </Select>
                </div>
              )}
              ...
              {parsedPrincipal > 5000 && (
                <div className="space-y-2">
                  <Label htmlFor="numMonths">Term Length (Months) - Big Loan</Label>
                  <Input
                    id="numMonths"
                    type="number"
                    placeholder="e.g. 6"
                  ...
                  <p className="text-xs text-text-secondary">
                    Big loans are amortized based on declining balance over {numMonths || "X"} months.
                  </p>
                </div>
              )}
            </div>
          ...
          {step === 3 && calcResult && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4 bg-bg-secondary p-4 rounded-lg">
                ...
                <div>
                  <p className="text-sm text-text-secondary">Total Interest Expected</p>
                  <p className="font-medium text-success">₱{calcResult.totalInterestExpected.toFixed(2)}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="border rounded-md divide-y overflow-hidden text-sm">
                  {calcResult.schedules.map((sch, i) => (
                    <div key={i} className="flex justify-between p-3 bg-background">
                      <span>{format(new Date(sch.expectedDate), "MMM do, yyyy")}</span>
                      <span className="font-medium">₱{sch.expectedAmount.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4 p-4 border border-border rounded-md bg-background">
                    <h4 className="font-medium text-sm text-text-primary">RC Allocation</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          step="0.01"
                          value={rcPercent}
                          onChange={(e) => handleCustomAllocationChange("RC_%", e.target.value)}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label>Amount (₱)</Label>
                  ...
                  {/* EDITH Group */}
                  <div className="space-y-4 p-4 border border-border rounded-md bg-background">
                    <h4 className="font-medium text-sm text-text-primary">EDITH Allocation</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label>Percentage (%)</Label>
                        <Input
                          ...
                          step="0.01"
                          value={edithPercent}
                          onChange={(e) => handleCustomAllocationChange("EDITH_%", e.target.value)}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label>Amount (₱)</Label>
                ...
                {allocationError && (
                  <p className="text-sm text-destructive font-medium">{allocationError}</p>
                )}
              </div>
            </div>
          )}
              ...
              <div className="rounded-lg border bg-card p-6 shadow-sm">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between py-1 border-b">
                    ...
                    <span className="font-medium">{selectedBorrower?.first_name} {selectedBorrower?.last_name}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b">
                    <span className="text-text-secondary">Category</span>
                    <Badge variant="outline">{isBigLoan ? "Big Loan" : "Small Loan"}</Badge>
                  </div>
                  <div className="flex justify-between py-1 border-b">
                    <span className="text-text-secondary">Principal Amount</span>
                    <span className="font-medium">₱{parsedPrincipal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b">
                    <span className="text-text-secondary">Term</span>
                    <span className="font-medium">{isBigLoan ? `${numMonths} Months` : termType}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b">
                    <span className="text-text-secondary">Total Interest Expected</span>
                    <span className="font-medium text-success">₱{calcResult?.totalInterestExpected.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b">
        ...
        <CardFooter className="flex justify-between">
          ...
          <Button
            ...
            disabled={
              isSubmitting ||
              (step === 1 && !borrowerId) ||
              (step === 2 && (!principalAmount || !releaseDate || (!isBigLoan && !termType) || (isBigLoan && !numMonths))) ||
